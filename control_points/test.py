'''
python test.py --data_path_test '../data/test_6'
'''
import os
import sys
sys.path.append("control_points/")
import argparse
import torch
...
def train(args):
    ...
    if args.resume is not None:
        ...
        reslut_file = open(path + '/' + date + date_time + '_' + _re_date + '_' + args.arch + '.log', 'w')
    else:
        _re_date = None
    ...
    model.eval()
    img_1, img_2 = FlatImg.validateOrTestModelV3(epoch, 0, validate_test='t_all')
    exit()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--arch', nargs='?', type=str, default='Document-Dewarping-with-Control-Points', help='Architecture')
    parser.add_argument('--img_shrink', nargs='?', type=int, default=None, help='short edge of the input image')
    parser.add_argument('--n_epoch', nargs='?', type=int, default=300, help='# of the epochs')
    parser.add_argument('--optimizer', type=str, default='adam', help='optimization')
    parser.add_argument('--l_rate', nargs='?', type=float, default=0.0002, help='Learning Rate')
    parser.add_argument('--print-freq', '-p', default=60, type=int, metavar='N', help='print frequency (default: 10)')  # print frequency
    parser.add_argument('--data_path_train', default=Path('dataset/fiducial1024/fiducial1024_v1/color/'), type=str, help='the path of train images.')  # train image path
    parser.add_argument('--data_path_validate', default=Path('dataset/fiducial1024/fiducial1024_v1/validate/'), type=str, help='the path of validate images.')  # validate image path
    parser.add_argument('--resume', default=Path('models/2021-02-03 16_15_55flat_img_by_fiducial_points-fiducial1024_v1.pkl'), type=str,  help='Path to previous saved model to restart from')    
    parser.add_argument('--batch_size', nargs='?', type=int, default=1, help='Batch Size')#28
    parser.add_argument('--schema', type=str, default='test', help='train or test')       # train  validate
    parser.add_argument('--parallel', default=None, type=list, help='choice the gpu id for parallel ')
    parser.add_argument('--output-path', default=Path('../data/result_control_points/', type=str, help='the path is used to  save output --img or result.')) 
    parser.add_argument('--data_path_test', default=Path('../data/test_3_2fps', type=str, help='the path of test images.'))
